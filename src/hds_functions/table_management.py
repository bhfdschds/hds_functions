"""Module for managing tables in a PySpark environment.

Functions:
  - load_table: Load a Spark table, optionally filtering by archive date and
        standardising columns.
  - save_table: Save a DataFrame to a database table.
  - get_archive_versions: Retrieve a list of unique archive versions.
  - standardise_table: Standardise the DataFrame based on the specified method,
        including renaming person ID variables, formatting dates as yyyy-MM-dd,
        and cleaning column names.
"""
...
def load_table(
    table: str, table_directory: str = None, method: str = None
) -> DataFrame:
    """Load a table, optionally filtering by archive date and standardising the data.

    Args:
        table (str): The name of the table to load.
        table_directory (str): Path to the JSON file containing table directories.
        method (str): The method used to standardise the table for column names
            (e.g., `person_id`). See `standardise_table` function for details.
    ...
    assert table in table_directory.keys(), (
        f"Table key '{table}' not found in table_directory"
    )
    ...
    assert (
        (archive_date is None and max_archive_date is None)
        or (archive_date is not None and max_archive_date is None)
        or (archive_date is None and max_archive_date is not None)
    ...
    if archive_date == "latest":
        archive_date_max = df.agg(
            f.max("archived_on").alias("_max_archive_date")
        ).collect()[0][0]
...
def save_table(df, table: str, table_directory=None, partition_by=None) -> None:
    ...
    assert (
        ...
        or all(isinstance(col, str) for col in partition_by)
...
def get_archive_versions(
    df: DataFrame, version_column: str = "archived_on"
) -> List[str]:
    """Get distinct version archive dates from the DataFrame.

    Parameters:
        df (DataFrame): The input DataFrame.
        version_column (str): The name of the column containing version information.
            Default is 'archived_on'.

    Returns:
        list[str]: A list of distinct archive versions.
    """
    return list(
        df.select(f.col(version_column).cast("string"))
        .distinct()
...
def standardise_table(df, method):
    """Standardise the DataFrame according to the specified method.

    In general, these steps are included:
    - Renaming Person ID Variables: Rename variables representing person IDs to
      'person_id' for consistency across datasets.
    - Date Formatting: Format date variables into yyyy-MM-dd format to ensure
      uniformity and facilitate analysis.
    - Cleaning Column Names: Standardise column names by converting to lowercase
      and replacing spaces with underscores.

    Args:
        df (pyspark.sql.DataFrame): The DataFrame to be standardised.
        method (str): The method used for standardisation. Valid methods include:
            'deaths', 'gdppr', 'hes_apc', 'hes_op', 'hes_ae', 'pillar_2', 'ssnap',
            'sgss', 'vaccine_status'.

    Returns:
        pyspark.sql.DataFrame: The standardised DataFrame.
    """
    # Map method to corresponding function
    method_functions = {
    ...
    # Validate method is supported, else raise informative error
    if method not in method_functions:
        raise ValueError(
            f"'{method}' is not a recognised standardise_table method. "
            f"Available methods: deaths, gdppr, hes_apc, hes_op, hes_ae, pillar_2, "
            f"ssnap, sgss, vaccine_status, primary_care_meds, chess"
        )

    # Call the appropriate standardisation function for the method
    return method_functions[method](df)


def standardise_deaths_table(df):
    """Standardised method for loading ONS Deaths table."""
    return (
        df.withColumnRenamed("DEC_CONF_NHS_NUMBER_CLEAN_DEID", "person_id")
        ...
        .withColumn(
            "date_of_death",
            f.when(
                f.col("date_of_death").rlike(r"\d{8}"),
...
def standardise_gdppr_table(df):
    """Standardised method for loading GDPPR table."""
    return df.withColumnRenamed("NHS_NUMBER_DEID", "person_id").transform(
        clean_column_names
    )


def standardise_hes_apc_table(df):
    """Standardised method for loading HES-APC table."""
    return df.withColumnRenamed("PERSON_ID_DEID", "person_id").transform(
        clean_column_names
    )


def standardise_hes_op_table(df):
    """Standardised method for loading HES-OP table."""
    return df.withColumnRenamed("PERSON_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_hes_ae_table(df):
    """Standardised method for loading HES-A&E table."""
    return df.withColumnRenamed("PERSON_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_pillar_2_table(df):
    """Standardised method for loading Pillar 2 table."""
    return df.withColumnRenamed("Person_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_ssnap_table(df):
    """Standardised method for loading SSNAP table."""
    return df.withColumnRenamed("Person_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_sgss_table(df):
    """Standardised method for loading SGSS table."""
    return df.withColumnRenamed("PERSON_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_vaccine_status_table(df):
    """Standardised method for loading COVID-19 Vaccination table."""
    return (
        df.withColumnRenamed("PERSON_ID_DEID", "person_id")
        .transform(clean_column_names)
        ...
        .withColumn(
            "date_and_time",
            f.to_timestamp(f.col("date_and_time"), "yyyyMMdd'T'HHmmssSS"),
        )
    )


def standardise_primary_care_meds_table(df):
    """Standardised method for loading Primary Care Dispensed Medications table."""
    return df.withColumnRenamed("Person_ID_DEID", "person_id").transform(
        clean_column_names
    )
...
def standardise_chess_table(df):
    """Standardised method for loading COVID-19 CHESS table."""
    return df.withColumnRenamed("PERSON_ID_DEID", "person_id").transform(
        clean_column_names
    )
